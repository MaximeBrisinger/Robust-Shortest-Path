...
function SP1(arcs, input_graph)
    ...
    while i <= length(arcs) && contin
        if quantity + arcs[i][4] <= input_graph.d1
            quantity += arcs[i][4]
            val_sp1 += arcs[i][3] * arcs[i][4] * arcs[i][5]
...
function cutting_planes(input_graph, dict_row, verbose, CPU_time_limit)
    ...
    while contin
        ...
        for a in input_graph.arcs 
            if x_val[(a[1], a[2])] <= 1 - eps && x_val[(a[1], a[2])] >= eps
                solution_is_int = false
            end
        end
        ...
        if !solution_is_int
            ...
            # reconstruction d'une solution entière
            ...
            path = []
            vertex = input_graph.s
            while vertex == input_graph.s || last(path)[2] != input_graph.t
                i = 1
                while i < length(arcs) && arcs[i][1] != vertex
                    i += 1                        
                end
                new_vertex = arcs[i][2]
                push!(path, (vertex, new_vertex))
                vertex = new_vertex
            end
            arcs = [(a[1], a[2], input_graph.traveltime_matrix[a[1], a[2]], input_graph.ceil_uncert_traveltime[a[1], a[2]], 1.0) for a in path]
        else
            arcs = [(a[1], a[2], input_graph.traveltime_matrix[a[1], a[2]], input_graph.ceil_uncert_traveltime[a[1], a[2]], x_val[(a[1], a[2])]) for a in input_graph.arcs if x_val[(a[1], a[2])] >= 1-eps] #TOD0: transformer en 1-eps
        end
    ...
    push!(dict_row, "solution_is_int" => solution_is_int)
